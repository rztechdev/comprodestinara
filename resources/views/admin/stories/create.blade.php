...
@section('title', isset($story) ? 'Edit Cerita' : 'Tulis Cerita Baru')

@section('content')
<div class="max-w-4xl mx-auto space-y-4">
    <div class="flex items-center justify-between">
        <h2 class="text-base font-semibold text-gray-800">{{ isset($story) ? 'Edit Cerita Lapangan' : 'Tulis Cerita Lapangan Baru' }}</h2>
    ...
    <form action="{{ isset($story) ? route('admin.stories.update', $story->id) : route('admin.stories.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-4 text-xs">
        @csrf
        @if(isset($story))
            @method('PUT')
        @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <input type="text" name="category" value="{{ old('category', $story->category ?? 'Etnobotani') }}" placeholder="cth. Etnobotani" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-[#703a3a]">
            </div>

            <div>
                <label class="block font-medium text-gray-700 mb-1">Nomor Arsip</label>
                <input type="text" name="archive_no" value="{{ old('archive_no', $story->archive_no ?? '') }}" placeholder="cth. Arsip Lapangan No. 042" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-[#703a3a]">
            </div>

            <div>
                <label class="block font-medium text-gray-700 mb-1">Estimasi Waktu Baca</label>
                <input type="text" name="read_time" value="{{ old('read_time', $story->read_time ?? '5 Menit') }}" placeholder="cth. 7 Menit Baca" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-[#703a3a]">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="author_role" value="{{ old('author_role', $story->author_role ?? '') }}" placeholder="cth. Peneliti Etnobotani" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-[#703a3a]">
            </div>
        </div>
        ...
        <div>
            <label class="block font-medium text-gray-700 mb-1">Foto Sampul Cerita</label>
        ...
        <div class="flex items-center gap-6 pt-2">
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $story->is_featured ?? false) ? 'checked' : '' }} class="rounded text-[#703a3a] focus:ring-0">
                <span class="font-medium text-gray-700">Cerita Unggulan (Featured)</span>
            </label>

            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $story->is_active ?? true) ? 'checked' : '' }} class="rounded text-[#703a3a] focus:ring-0">
        ...
        <div class="pt-4 border-t border-gray-100 flex justify-end gap-3">
            ...
            <button type="submit" class="px-5 py-2 bg-[#703a3a] text-white font-medium rounded-lg hover:bg-[#582d2d] shadow-sm">
                {{ isset($story) ? 'Perbarui Cerita' : 'Terbitkan Cerita' }}
            </button>
        </div>
    </form>
</div>
