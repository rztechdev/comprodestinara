...
<div class="max-w-4xl mx-auto space-y-4">
    <div class="flex items-center justify-between">
        <h2 class="text-base font-semibold text-gray-800">{{ isset($destination) ? 'Edit Tapak Destinasi' : 'Tambah Tapak Destinasi Baru' }}</h2>
    ...
    <form action="{{ isset($destination) ? route('admin.destinations.update', $destination->id) : route('admin.destinations.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-4 text-xs">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="location" value="{{ old('location', $destination->location ?? '') }}" placeholder="cth. Gunungkidul, D.I. Yogyakarta" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-[#703a3a]">
            </div>

            <div>
                <label class="block font-medium text-gray-700 mb-1">Label / Badge</label>
                <input type="text" name="badge" value="{{ old('badge', $destination->badge ?? '') }}" placeholder="cth. Konservasi Karst" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-[#703a3a]">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="module_name" value="{{ old('module_name', $destination->module_name ?? '') }}" placeholder="cth. Hidrologi Karst &amp; Adat" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-[#703a3a]">
            </div>

            <div>
                <label class="block font-medium text-gray-700 mb-1">Kapasitas Maksimal Rombongan</label>
                <input type="text" name="capacity" value="{{ old('capacity', $destination->capacity ?? '') }}" placeholder="cth. 30 Peneliti" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-[#703a3a]">
            </div>
        </div>
        ...
        <div>
            <label class="block font-medium text-gray-700 mb-1">Foto Utama (Upload / Update)</label>
        ...
        <div class="flex items-center gap-6 pt-2">
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $destination->is_featured ?? false) ? 'checked' : '' }} class="rounded text-[#703a3a] focus:ring-0">
                <span class="font-medium text-gray-700">Tampilkan di Beranda (Featured)</span>
            </label>

            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $destination->is_active ?? true) ? 'checked' : '' }} class="rounded text-[#703a3a] focus:ring-0">
